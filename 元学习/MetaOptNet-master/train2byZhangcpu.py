...
def one_hot(indices, depth):
    ...
    encoded_indicies = torch.zeros(indices.size() + torch.Size([depth])).cpu()
...
def get_model(options):
    ...
    if options.network == 'ProtoNet':
        network = ProtoNetEmbedding().cpu()
    elif options.network == 'R2D2':
        network = R2D2Embedding().cpu()
    elif options.network == 'ResNet':
        if options.dataset == 'miniImageNet' or options.dataset == 'tieredImageNet':
            network = resnet12(avg_pool=False, drop_rate=0.1, dropblock_size=5).cpu()
            network = torch.nn.DataParallel(network, device_ids=[0, 1, 2, 3])
        else:
            network = resnet12(avg_pool=False, drop_rate=0.1, dropblock_size=2).cpu()
    else:
    ...
    if options.head == 'ProtoNet':
        cls_head = ClassificationHead(base_learner='ProtoNet').cpu()
    elif options.head == 'Ridge':
        cls_head = ClassificationHead(base_learner='Ridge').cpu()
    elif options.head == 'R2D2':
        cls_head = ClassificationHead(base_learner='R2D2').cpu()
    elif options.head == 'SVM':
        cls_head = ClassificationHead(base_learner='SVM-CS').cpu()
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save-path', default='miniImageNet_MetaOptNet_SVM')
    ...
    for epoch in range(1, opt.num_epoch + 1):
        ...
        for i, batch in enumerate(tqdm(dloader_train(epoch)), 1):
            data_support, labels_support, data_query, labels_query, _, _ = [x.cpu() for x in batch]
...
# 下面都是一些模型保存.  存太多遍了, 我们改成之存最终模型就得了.
